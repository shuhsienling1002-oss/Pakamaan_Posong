...
class StrategyEngine:
    ...
    def check_connection_safety(self, hour, county):
        """邏輯 2-1: 轉乘安全閥 (檢查時間是否太晚)"""
        # 假設：桃園->台北(1h) + 轉乘(0.5h) + 台北->羅東(1.5h) + 緩衝(0.5h) = 約 3.5 小時後抵達羅東
        arrival_luodong = hour + 3.5
        
        # 羅東往花蓮末班車約 22:30 / 往台東末班車約 19:30 (保守估計)
        if county == "台東縣":
            if arrival_luodong > 19.5: # 16:00 出發
                return False, "⚠️ **太晚了！** 您抵達羅東時，往台東的火車可能已收班。請務必在 **16:00 前** 出發。"
        else: # 花蓮縣
            if arrival_luodong > 22.5: # 19:00 出發
    ...
    def get_no_ticket_strategy(self, township, is_safe, warning_msg):
        ...
        if is_safe:
            ...
            plans.append({
                ...
                "route": "此路不通",
                "desc": warning_msg,
                "steps": ["請改搭計程車或在羅東住宿一晚。", "或者改為 **高鐵南迴** 方案（若往南）。"],
                "tags": ["危險"]
            })
