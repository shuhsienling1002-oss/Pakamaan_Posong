...
class StrategyEngine:
    ...
    def get_transfer_strategy(self, township, hour, county):
        ...
        # [修正] 修改標題與描述，將「逆向」改為「先往北搭」
        plans.append({
            "title": "🚆 方案 B: 先往北搭 (樹林始發)", 
            "icon": "🛡️",
            "desc": "不要在桃園等！買票**先往北搭到樹林站** (東部幹線起點)，直接上車搶自由座/站票。",
...
def main_app():
    ...
    with st.container():
        ...
        with c1: county = st.selectbox("📍 回哪裡？", ["花蓮縣", "台東縣"])
        with c2: township = st.selectbox("🏠 鄉鎮", TOWNSHIP_DB[county])
        with c3: date_str = st.selectbox("📅 日期", ["2/13 (五) 假前一天", "2/14 (六) 連假首日", "2/15 (日) 小年夜", "2/16 (一) 除夕"])
    ...
    with tab1:
        ...
        if st.button("🚀 分析路況", key="btn_drive", type="primary"):
            ...
            st.markdown(f"<div class='info-card'><b>💡 策略建議：</b><br>{report['desc']}</div>", unsafe_allow_html=True)
            if report['hov_warning']: st.error(report['hov_warning'])
            base_time = 3.5 + (1.0 if "台東" in county else 0)
            st.caption(f"🏁 預估抵達 {township} 耗時: 約 {(base_time * report['jam_factor']):.1f} 小時")
        ...
        if st.button("🚑 尋找替代方案", key="btn_no_ticket", type="primary"):
            ...
            for plan in strategies:
                with st.expander(f"{plan['icon']} {plan['title']}", expanded=("推薦" in plan['title'])):
                    ...
                    st.info(plan['desc'])
                    if plan['risk'] == "極高": st.warning("⚠️ 此路徑風險極高，請勿嘗試")

    with tab3:
        st.write("#### 🚄 參考車次 (桃園出發)")
        st.caption("僅列出熱門直達車次，完整資訊請以台鐵官網為準。")
        df = pd.DataFrame(TRAIN_DATA)
        st.dataframe(df, hide_index=True, use_container_width=True, column_config={"特徵": st.column_config.TextColumn("特徵", help="班次特性")})
        st.markdown("---")
        st.markdown(f"**🚌 抵達 {township} 後轉乘：**")
        if "玉里" in township or "富里" in township: st.success("💡 建議搭到 **玉里站**，站前租車或轉乘最方便。")
        elif "豐濱" in township: st.success("💡 需在花蓮站轉搭 **1140/1145 客運** (海線)。")
        else: st.success("💡 車站前計程車充足，或請家人騎車來載。")
